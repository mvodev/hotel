...
$('.range-slider').ionRangeSlider({
 ...
 onChange: function (data) {
  let minValue = ''
  let maxValue = ''
  if (data.from >= 1000) minValue = data.from.substring(0, 1) + ' ' + (data.from.slice(1));
  else minValue = data.from;
  if (data.to >= 1000) maxValue = data.from.substring(0, 1) + ' ' + (data.from.slice(1));
...
/******/ });
//# sourceMappingURL=main.d41436548cc98f5a982b.js.map